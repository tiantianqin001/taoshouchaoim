...
public class ConversationCommonHolder extends ConversationBaseHolder {
    ...
    public void layoutViews(ConversationInfo conversation, int position) {
        ...
        if (draftInfo != null) {
            ...
            // 如果最后一条消息是自定义消息, 获取要显示的字符 todo 这个是获取群聊天的最后一条消息
            ...
            //自定义消息不一样 主要是拉群
            V2TIMMessage conversationLastMessage = conversation.getLastMessage();
            if (conversationLastMessage!=null){
                V2TIMCustomElem customElem1 = conversationLastMessage.getCustomElem();
                if (customElem1!=null){
                    byte[] data = customElem1.getData();
                    String s = new String(data);
                    try {
                        JSONObject jsonObject = new JSONObject(s);
                        String opUser = jsonObject.optString("opUser");
                        String content = jsonObject.optString("content");
                        String businessID = jsonObject.optString("businessID");
                        if (businessID.equals("group_create")){
                            messageText.setText(opUser+content);
