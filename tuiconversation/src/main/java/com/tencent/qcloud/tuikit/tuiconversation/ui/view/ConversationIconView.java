...
public class ConversationIconView extends RelativeLayout {
     ...
     * 设置会话头像的url
     *
     * @param iconUrlss 头像url,最多只取前9个
     */
    public void setIconUrls( List<Object> iconUrlss, final String conversationId) {

        if (mIconView instanceof SynthesizedImageView) {
            ((SynthesizedImageView) (mIconView)).displayImage(iconUrlss).load(conversationId);
        }
