...
public class FriendProfileLayout extends LinearLayout implements View.OnClickListener, IFriendProfileLayout {
    ...
    private String join_protect;
    private int groupMemberProtect;
    private TextView tv_contact_set_add_wording;
    ...
    private void init() {
        ...
        tv_contact_set_add_wording = findViewById(R.id.tv_contact_set_add_wording);
        ...
        mTitleBar.setBackgroundColor(getContext().getResources().getColor(R.color.white));
        mTitleBar.setTitle(getResources().getString(R.string.profile_detail), ITitleBarLayout.Position.MIDDLE);
        mTitleBar.getLeftIcon().setVisibility(VISIBLE);
        ...
        join_protect = sp.getString("join_protect", "");
    ...
    private void setViewContentFromGroupInfo(GroupInfo groupInfo) {
        ...
        //如果已经设置了群保护模式 在不是好友的情况就不能添加好友了
        //必须是群才隐藏
        if(isQuestGroup && groupMemberProtect==1){
            if (isAdmin){
        ...
        if (TextUtils.equals(mContactInfo.getId(), TUILogin.getLoginUser())) {
            ...
            if (mContactInfo.isBlackList()) {
                ...
                if (!isFriend) {
                    ...
                    mTitleBar.getMiddleTitle().setTextColor(getResources().getColor(R.color.add_friend));
                    // mTitleBar.getMiddleTitle().setTextSize(getResources().getDimension(R.dimen.add_friend));
                    //如果已经设置了群保护模式 在不是好友的情况就不能添加好友了
                    //必须是群才隐藏
                    if(isQuestGroup && groupMemberProtect==1 ){
                        if (isAdmin){
                    ...
                    if (!isGroup){
                        addFriendArea.setVisibility(VISIBLE);
                        tv_contact_set_add_wording.setVisibility(VISIBLE);
                        addWordingEditText.setVisibility(VISIBLE);
                        remarkAndGroupTip.setVisibility(VISIBLE);
                        addFriendGroupLv.setVisibility(VISIBLE);
    ...
    private boolean isQuestGroup;
    public void setIsGroup(boolean isGroup) {
        isQuestGroup = isGroup;
    }

    public void setGroupMemberProtect(int groupMemberProtect) {

        this.groupMemberProtect = groupMemberProtect;
    }
