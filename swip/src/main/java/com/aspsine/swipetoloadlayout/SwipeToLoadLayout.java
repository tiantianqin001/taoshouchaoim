...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    public SwipeToLoadLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                } else if (attr == R.styleable.SwipeToLoadLayout_refresh_complete_trigger_offset) {
                    setCompleteOffset(a.getDimensionPixelOffset(attr, 0));
    ...
    //TODO:
    public void setHasLoadAll(boolean loadAll) {
        Log.v(TAG, "setHasLoadAll:" + loadAll);
        if (mLoadMoreCallback != null) {
            mLoadMoreCallback.setLoadAll(loadAll);
        }
    }
    ...
    public void setCompleteOffset(int offset) {
        this.mCompleteOffset = offset;
    }
     ...
     * @param refreshing
     */
    public void setRefreshing(boolean refreshing, int dataSize) {
        setRefreshing(refreshing, dataSize, "");
    }
     ...
     * @param refreshing
     */
    public void setRefreshing(boolean refreshing, String toast) {
        setRefreshing(refreshing, 0, toast);
    }
     ...
     * @param refreshing
     */
    public void setRefreshing(boolean refreshing, final int dataSize, final String toast) {
        ...
        if (refreshing) {
            ...
            if (STATUS.isRefreshing(mStatus)) {
                if (mRefreshCallback.isShowPic() && dataSize == 0 && TextUtils.isEmpty(toast)) {
                    //图片类型，无显示数据，则直接结束
                    setStatus(STATUS.STATUS_SWIPING_TO_COMPLETE);
                    scrollRefreshingToDefault();
                    return;
                }
                scrollRefreshingToComplete();
                mRefreshCallback.onComplete(dataSize, toast);
                //默认等待300毫秒显示，如果显示图片则使用用户自定义时间
                int delay = DEFAULT_REFRESH_COMPLETE_DELAY_DURATION;
                if (mRefreshCallback.isShowPic()) {
                    delay = mRefreshCompleteDelayDuration;
                }
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        scrollCompleteToDefault(dataSize, toast);
                    }
                }, delay);
            }
        }
     ...
     * @param loadingMore
     */
    public void setLoadingMore(boolean loadingMore, int dataSize) {
        setLoadingMore(loadingMore, dataSize, "");
    }
     ...
     * @param loadingMore
     */
    public void setLoadingMore(boolean loadingMore, String toast) {
        setLoadingMore(loadingMore, 0, toast);
    }
     ...
     * @param loadingMore
     */
    public void setLoadingMore(boolean loadingMore, int dataSize, String toast) {
        ...
        if (loadingMore) {
            ...
            if (STATUS.isLoadingMore(mStatus)) {
                mLoadMoreCallback.onComplete(dataSize, toast);
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
    ...
    protected boolean canChildScrollUp() {
        ...
        if (mTargetView instanceof ScrollView || mTargetView instanceof AbsListView || mTargetView instanceof ScrollingView) {
            scrollingView = mTargetView;
        } else if (mTargetView instanceof ViewGroup) {
            scrollingView = getScrollingView((ViewGroup) mTargetView);
        }
     ...
     * @return
     */
    private View getScrollingView(ViewGroup parent) {
        int childCount = parent.getChildCount();
        for (int i = 0; i < childCount; i++) {
            View child = parent.getChildAt(i);
            if (child instanceof ScrollView || child instanceof AbsListView || child instanceof ScrollingView) {
                return child;
            } else if (child instanceof ViewGroup) {
                View view = getScrollingView((ViewGroup) child);
                if (view != null) {
                    return view;
                }
            }
    ...
    private void fixCurrentStatusLayout() {
        if (STATUS.isRefreshing(mStatus)) {
            ...
            layoutChildren();
            invalidate();
        } else if (STATUS.isComplete(mStatus)) {
            mTargetOffset = (int) (mCompleteOffset + 0.5f);
            mHeaderOffset = mTargetOffset;
            mFooterOffset = 0;
            layoutChildren();
            invalidate();
    ...
    private void scrollDefaultToRefreshing() {
        Log.d(TAG, "scrollDefaultToRefreshing : " + mRefreshTriggerOffset);
    ...
    private void scrollDefaultToLoadingMore() {
        Log.d(TAG, "scrollDefaultToLoadingMore");
    ...
    private void scrollSwipingToRefreshToDefault() {
        Log.d(TAG, "scrollSwipingToRefreshToDefault");
    ...
    private void scrollSwipingToLoadMoreToDefault() {
        Log.d(TAG, "scrollSwipingToLoadMoreToDefault");
    ...
    private void scrollReleaseToRefreshToRefreshing() {
        Log.d(TAG, "scrollReleaseToRefreshToRefreshing mHeaderHeight : " + mHeaderHeight + " mHeaderOffset : " + mHeaderOffset);
    ...
    private void scrollReleaseToLoadMoreToLoadingMore() {
        Log.d(TAG, "scrollReleaseToLoadMoreToLoadingMore");
        mAutoScroller.autoScroll(-mFooterOffset - mFooterHeight, mReleaseToLoadMoreToLoadingMoreScrollingDuration);
    }

    private void scrollRefreshingToComplete() {
        Log.d(TAG, "scrollRefreshingToComplete");
        mAutoScroller.autoScroll(mCompleteOffset - mHeaderHeight, 1);
    }

    private void scrollCompleteToDefault(int dataSize, String toast) {
        int duration = DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION;
        if (mRefreshCallback.isShowPic()) {
            duration = mRefreshCompleteToDefaultScrollingDuration;
            if (dataSize == 0 && TextUtils.isEmpty(toast)) {
                duration = 1;
            }
        }
        Log.d(TAG, "scrollCompleteToDefault : " + duration);
        mAutoScroller.autoScroll(-mCompleteOffset, duration);
    }

    private void scrollRefreshingToDefault() {
        Log.d(TAG, "scrollRefreshingToDefault");
        mAutoScroller.autoScroll(-mHeaderHeight, mDefaultToRefreshingScrollingDuration);
    }

    private void scrollLoadingMoreToDefault() {
        Log.d(TAG, "scrollLoadingMoreToDefault");
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onComplete(int dataSize, String toast) {
            if (mHeaderView != null && mHeaderView instanceof SwipeTrigger) {
                ((SwipeTrigger) mHeaderView).onComplete(dataSize, toast);
            }
        }
        ...
        public boolean isShowPic() {
            if (mHeaderView != null && mHeaderView instanceof SwipeTrigger) {
                return ((SwipeTrigger) mHeaderView).isShowPic();
            }
            return false;
        }
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        public void setLoadAll(boolean hasLoadAll) {
            if (mFooterView != null && mFooterView instanceof SwipeLoadMoreTrigger) {
                ((SwipeLoadMoreTrigger) mFooterView).setLoadAll(hasLoadAll);
            }
        }
        ...
        public void onComplete(int dataSize, String toast) {
            if (mFooterView != null && mFooterView instanceof SwipeTrigger) {
                ((SwipeTrigger) mFooterView).onComplete(dataSize, toast);
            }
        }
        ...
        public boolean isShowPic() {
            if (mFooterView != null && mFooterView instanceof SwipeTrigger) {
                return ((SwipeTrigger) mFooterView).isShowPic();
            }
            return false;
        }
    ...
    private final static class STATUS {
        private static final int STATUS_SWIPING_TO_COMPLETE = -5;
        ...
        private static boolean isComplete(final int status) {
            return status == STATUS.STATUS_SWIPING_TO_COMPLETE;
        }
