...
public class GroupMemberLayout extends LinearLayout implements IGroupMemberLayout {
    ...
    private void init() {
        ...
        mAdapter = new GroupMemberAdapter(getContext());
    ...
    List<GroupMemberInfo> memberNewDetails = new ArrayList<>();
    public void onGroupInfoChanged(GroupInfo groupInfo) {
        //在群管理的时候取掉群主
        if (is_group_manage){
            memberNewDetails.clear();
            List<GroupMemberInfo> memberDetails = groupInfo.getMemberDetails();
            for (GroupMemberInfo memberDetail : memberDetails) {
                Log.i("", "onGroupInfoChanged: "+memberDetail);
                String account = memberDetail.getAccount();
                if (!account.equals(groupInfo.getOwner())){
                    memberNewDetails.add(memberDetail);
                }
            }
            groupInfo.setMemberDetails(memberNewDetails);
    ...
    public void setIsManage(boolean is_group_manage) {

        this.is_group_manage = is_group_manage;
    }
}
